...
def process_data(dataset):
    ...
    # DO LOG
    for k in ['n_jobs', 'max_iter', 'n_samples', 'n_features', 'complex', 'newtime']:
        dataset[k] = dataset[k].apply(lambda x: log10(x))

    # expect to learn a linear model
    dataset = dataset[["alpha", "newtime", "elasticnet", "l1", "l2", "none"]]
...
if __name__ == "__main__":
    data = pd.read_csv("./data/generated/new_train_data.csv")
    test_data = pd.read_csv("./data/generated/new_test_data.csv")
    print(data.columns)
    ...
    # regressor = SVR(kernel='rbf', gamma='auto', C=1, epsilon=0.01, tol=1e-4)
    regressor = AdaBoostRegressor(base_estimator=regressor, n_estimators=50, learning_rate=0.05, loss='linear')
