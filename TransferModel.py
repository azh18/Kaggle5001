...
if __name__ == "__main__":
    data_origin = pd.read_csv("./data/generated/new_train_data.csv").drop(columns=["time"]).rename(columns={"newtime": "time"})
    print(data_origin.columns)
    data0 = pd.read_csv("./data/generated/time_mode0.csv")
    print(data0.columns)
    data1 = pd.read_csv("./data/generated/time_mode1.csv")
    data2 = pd.read_csv("./data/generated/time_mode2.csv")
    data3 = pd.read_csv("./data/generated/time_mode3.csv")
    data = pd.concat([data_origin, data0, data1, data2, data3], axis=0)
    ...
    regressor = MLPRegressor(hidden_layer_sizes=(84, 42, 21), alpha=0.005, learning_rate='adaptive',
                             early_stopping=True, n_iter_no_change=50, max_iter=100000, verbose=True)
    # regressor = SVR(kernel='rbf', gamma='auto', C=100, epsilon=0.01, tol=1e-4)
    regressor = AdaBoostRegressor(base_estimator=regressor, n_estimators=50, learning_rate=0.05, loss='exponential')
